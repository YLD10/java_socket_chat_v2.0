 ...
 * 
 * @Author : YLD
 * @CreateTime : 2017/12/2 00:07
 * @LastModifTime : 2018/8/30 15:30
 * @Mean: 某一位用户的客户端主程序，主要负责接收服务端发来的消息并进行处理，之后呈现给用户
 * 
 */
public class Client extends JFrame implements ActionListener {
    private static final long serialVersionUID = 8668382011408447330L;
    // 界面美化-start
    ...
    private void getDataServer() throws IOException {
        StringBuilder sBuilder = new StringBuilder("@#data#@"); // 消息载体
        ...
        while (true) { // 循环等待消息
            ...
            while ((str = getBr().readLine()) != null) { // 循环接收消息直至一则消息接收完毕
            ...
            if (!exit && flag) { // 如果是对话数据类型的消息，flag为true
                int end = sBuilder.indexOf("<>?:\"|{}"); // 发信人昵称字符串的结束下标
                String senName = sBuilder.substring(8, end); // 发信人昵称,8起始是为跳过消息首部
                // end+8为跳过"<>?:\"|{}"，在此之后就是对话的真正内容，尾部在之前被丢弃了
                String message = sBuilder.substring(end + 8);

                // 为调试程序而设置的打印消息的一个变量
                StringBuilder sb = new StringBuilder("消息接收成功！消息如下：\n\n==startMessage==\n发信人：");
                ...
                sb.delete(0, sb.length()); // 重复利用sb，存放对话显示窗口中的一条对话数据
                sb.append(senName); // 拼接发信人昵称
                sb.append(" "); // 拼接空格
                // 拼接对话日期
